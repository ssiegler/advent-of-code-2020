...
fn main() {
    ...
    let input = BufReader::new(input)
        .lines().map(|line| line.expect("Failed to read line").parse::<i32>().expect("failed to parse line as number"));
    let (a, b) = find_first_pair_with_sum(input, TARGET_SUM).expect("No pair found");
    println!("{} * {} = {}", a, b, a * b);
}

fn find_first_pair_with_sum<I>(numbers: I, sum: i32) -> Option<(i32, i32)>
...
mod tests {
    ...
    fn finds_first_pair_for_sum() {
        let numbers = vec![1721,
                           979,
                           366,
                           299,
                           675,
                           1456];
        assert_eq!(find_first_pair_with_sum(numbers.into_iter(), TARGET_SUM), Some((1721, 299)));
    }
}
