...
pub fn read_numbers_from_file(path: &Path) -> Vec<i32> {
    ...
    let input: Vec<i32> = BufReader::new(input)
        ...
        .map(|line| line.parse::<i32>().unwrap_or_else(|_| panic!("failed to parse line as number: {:?}", &line)))
        .collect();
    input
}
...
", i32::max_value(), i32::min_value())?;
        ...
        assert_eq!(read_numbers_from_file(file.path()), &[1, 2, 12, 13, i32::max_value(), 1000, -1, i32::min_value()]);
